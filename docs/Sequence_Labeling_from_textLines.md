...
- **アイテム（Item）**：アイテム名、数量（Quantity）、単価、合計価格
- **日付**
- **店舗名**
...
- `sum(items) == subtotal` または `sum(items) == total` の整合性チェック
- `tax breakdown` から `tax` や `total` を再計算して矛盾検知
- アイテム合計から `subtotal` や `total` を予測して候補として追加
...
- **アイテムは中央〜左側に名前、右側に価格が配置されることが多い**
- **数量は "×2" や "QTY: 2" のようにアイテム名と価格の間にあることが多い**
...
- `B-VAT_BASE`（課税対象額）
- `B-ITEM_NAME`（アイテム名）
- `B-ITEM_QUANTITY`（数量：×2, QTY: 2など）
- `B-ITEM_PRICE`（単価）
- `B-ITEM_TOTAL_PRICE`（アイテム合計価格）
- `B-DATE`
- `B-MERCHANT`
...
- `has_quantity_marker`（×, x, QTY, quantity, 数量など）
- `has_item_like`（文字列 + 空白 + 金額のパターン）
- `digit_count`, `alpha_count`
- `contains_colon`
- `is_middle_section`（y_centerが0.3〜0.7の範囲：アイテムリストが配置されやすい）
...
- `DATE`：日付正規表現にマッチ
- `ITEM_NAME`：文字列が含まれ、かつ `total|subtotal|vat|tax|payment` などのキーワードを含まない行
- `ITEM_QUANTITY`：`/(\d+)\s*[×x]|qty[:\s]*(\d+)/i` にマッチする数値
- `ITEM_TOTAL_PRICE`：アイテム行の右側（x_center > 0.6）にある金額
- `ITEM_PRICE`：数量がある場合、`ITEM_TOTAL_PRICE / ITEM_QUANTITY` で計算

**アイテム検出のルール例：**
1. セクションガード：`items` で始まる行から `subtotal|total|vat|tax|payment` で始まる行までをアイテムセクションと判定
2. フォールバック：セクションガードがない場合、中央領域（y_center: 0.3〜0.7）で、`total|subtotal|vat|tax|payment` を含まない行をアイテム候補とする
3. 数量の検出：
   - `×` または `x` マーク：`アイテム名 + 数量 + × + 単価 + 合計価格`
   - `QTY` ラベル：`アイテム名 + QTY: 数量 + 価格`
   - 数量なし：`アイテム名 + 価格`（数量=1と仮定）
...
**アイテム合計による整合性（重要）：**
- `items_sum = sum(all_item_total_prices)` を計算
- `|items_sum - subtotal|` が小さいほど加点（Subtotalがアイテム合計と一致）
- `|items_sum - total|` が小さいほど加点（Totalがアイテム合計と一致、税込みの場合）
- `|items_sum + tax - total|` が小さいほど加点（Total = アイテム合計 + 税）
- アイテム合計から `subtotal` や `total` を予測し、候補として追加

**位置情報による優先順位：**
- `total` はレシート下側にあるほど加点
- `total` のラベル行に `TOTAL` キーワードが近いほど加点
- アイテムは中央領域（y_center: 0.3〜0.7）にあるほど加点
...
- まずは既存のregex抽出を強化しつつ、
- 候補複数保持＋整合性スコアで選ぶ
- **アイテム抽出もルールベースで実装**（正規表現 + 位置情報）
- **アイテム合計を計算し、Subtotal/Totalの候補として追加**
...
| **アイテム名に数字が含まれる**（例："Item 123"） | 位置情報（右側の金額）と数量マーカー（×, QTY）で区別 |
| **数量の表記が多様**（×2, x2, QTY: 2, 2個など） | 多言語キーワードと正規表現で柔軟に対応 |
| **アイテム名と価格が別行** | Y座標が近い行を結合して1つのアイテムとして認識 |
| **アイテム合計とSubtotal/Totalが一致しない** | 税込み/税抜きの違いを考慮し、整合性スコアで最適な候補を選択 |
...
- `TAX_VALUE`
- `ITEM_NAME`（アイテム名）
- `ITEM_QUANTITY`（数量：オプション）
- `ITEM_TOTAL_PRICE`（アイテム合計価格）
- `DATE`
- `MERCHANT`

**アイテムラベルの詳細：**
- `ITEM_NAME`：アイテム名の行全体、または左側部分
- `ITEM_QUANTITY`：数量が明示されている場合（×2, QTY: 2など）
- `ITEM_TOTAL_PRICE`：アイテムの合計価格（右側に配置されることが多い）

**数量の扱い：**
- 数量が検出できない場合、`ITEM_QUANTITY = 1` と仮定
- 数量がある場合、`ITEM_PRICE = ITEM_TOTAL_PRICE / ITEM_QUANTITY` で計算可能
...
- ✅ **アイテム抽出もシーケンスラベリングで行うことで、多様なフォーマット（×マーク、QTYラベル、順序の違い）に対応可能**
- ✅ **アイテム合計による整合性チェックで、Subtotal/Totalの検出精度が向上**
- ✅ 学習データは最初から完璧に作らず、**ルール→擬似ラベル→改善**で現実的に回せる  
- ✅ TFLiteでオンデバイス推論でき、EUでもプライバシー面で強い（画像/テキストを外に出さない設計が可能）

### 12.1 現在の実装状況（2024年時点）

**実装済み：**
- ✅ `textLines` の構造化データ取得（`OCRResult.textLines`）
- ✅ 位置情報（bounding box）の活用
- ✅ 複数候補の収集と整合性スコアリング（`AmountCandidate`, `FieldCandidates`, `ConsistencyResult`）
- ✅ テーブルと非テーブルの統合的な整合性チェック
- ✅ ルールベースのアイテム抽出（`_extractItems`）

**今後の実装予定：**
- 🔄 アイテム抽出のシーケンスラベリング化
- 🔄 アイテム合計による整合性チェック（`items_sum` を `ConsistencyResult` に追加）
- 🔄 アイテム合計からSubtotal/Totalを予測して候補として追加
- 🔄 MLモデルの学習とTFLite統合
